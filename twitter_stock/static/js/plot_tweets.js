...
function computeLaplace(values,normalized) {
  var mu = 0;
  var b = 0;
  var times = [];
  var laplaces = [];
  ...
  for (var i = 0; i<values.length-1; i++) {
    laplaces.push(Math.exp(-Math.abs(i-mu)/b)/(2*b));
  }
  if (normalized) {
    laplaces = normalize(laplaces,0);
  }
  return laplaces;
}
...
document.forms.rangeform.addEventListener('change', function(e) {
  ...
  for (var i = 1; i < dates.length; i++) { 
    if (Date.parse(dates[i])>=fromDate & Date.parse(dates[i])<=toDate ) {
      ...
      //keep_laplaces.push(laplaces[i]);
    }
  }
  var keep_laplaces = computeLaplace(keep_values,true);
  keep_laplaces.unshift(laplaces[0]);
  var thisData = [keep_dates,keep_values,keep_laplaces]; 
...
var keep_laplaces = computeLaplace(norm_values,false);
keep_laplaces.unshift(laplace[0]);
myData[2] = keep_laplaces;
