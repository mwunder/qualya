...
$(function(callback) {
    ...
    function binsOverTime(bins,sents) {
        ...
        for(var i=0; i<bins[0].length; i++) { series.push([]) }

        for(var i=0; i<bins.length; i++) {

            for(var j=0; j<series.length; j++) {

                if (j<2) { series[j].push(-bins[i][j]) } else { series[j].push(bins[i][j]) }
            }
        }
      
        for(var j=0; j<series.length; j++) { series[j].unshift(sents[j]) }
    ...
    var bin_series = binsOverTime(bins, ['Strong_neg','Weak_neg','Neutral','Weak_pos','Strong_pos']);
